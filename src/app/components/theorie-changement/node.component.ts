...
@Component({
  ...
  <div editable="true" class="node" [style.left.px]="node.left || 80" [style.top.px]="node.top || 90" [style.background-color]="node.color"  
         id="{{node.id}}" >
        <textarea  name="name" id="{{node.id+'_content'}}" [style.height]="node.height" [style.width]="node.width"  value="{{node.name}}" style="background-color: transparent; border:1px solid transparent; wordWrap :break-word; resize: both;">
        </textarea>
...
export class NodeComponent2 implements AfterViewInit {
  @Input() node!: Node;
  ...
  ngAfterViewInit() {
    ...
    let Endpoint1 = {
      ...
      connectorStyle: { stroke: 'rgb(48, 121, 231)', strokeWidth: 3 },
      // connector: ['Bezier', { curviness: 63 }],
      connector: ['Straight'],
      maxConnections: 30,
      isTarget: false,

      connectorOverlays: [['Arrow', { location: 1, label: "foo", id: "label" }]],
