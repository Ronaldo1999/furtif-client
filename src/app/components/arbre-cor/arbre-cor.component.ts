...
import { NodeServiceArbre } from '../arbre/node.service';
import { Arbre } from 'src/app/class/arbre';
import { ArbreNiveau } from 'src/app/class/arbre-niveau';
import { ApiService } from 'src/app/services/api.service';

@Component({
  selector: 'app-arbre-cor',
  templateUrl: './arbre-cor.component.html',
  styleUrls: ['./arbre-cor.component.scss']
})

export class ArbreCorComponent implements OnInit {
  ...
  constructor(
    public nodeService: NodeServiceArbreCor,
    public nodeServiceForUpdate: NodeServiceArbre,
  ...
  ngOnInit() {
    ...
    if (this.route.snapshot.params.liens && this.route.snapshot.params.bascule == '1') {
      console.log(this.route.snapshot.params.liens);
      this.loadNodes(this.route.snapshot.params.liens);
    } else if (this.route.snapshot.params.liens && this.route.snapshot.params.bascule != '1') {
      console.log(this.route.snapshot.params.liens);
  ...
  loadNodes(liens: string) {
    this.nodeServiceForUpdate.removeConnexions(this.route.snapshot.params.liens);
    console.log(liens);
    const data = JSON.parse(liens);
    this.connections = data.connections;
  ...
  updateNode(element: any) {
    ...
    console.log(element);
    if (element.color == "rgb(67, 130, 179)" || element.color == "#4382B3") {
      element.color = "#C70039";
      this.nodeServiceForUpdate.addDynamicNodeAbre(element);
    } else if (element.color == "rgb(64, 124, 114)" || element.color == "#407C72") {
      element.color = "#581845";
      this.nodeServiceForUpdate.addDynamicNodeAbre(element);
    } else if (element.color == "rgb(242, 113, 62)" || element.color == "#F2713E") {
      element.color = "#FF5733";
      this.nodeServiceForUpdate.addDynamicNodeAbre(element);
    } else if (element.color == "rgb(114, 130, 114)" || element.color == "#728272") {
      element.color = "#4F9F9B";
      this.nodeServiceForUpdate.addDynamicNodeAbre(element);
    } else if (element.color == "rgb(70, 87, 94)" || element.color == "#46575E") {
      element.color = "#036FAC";
      this.nodeServiceForUpdate.addDynamicNodeAbre(element);
    } else if (element.color == "rgb(104, 221, 238)" || element.color == "#68DDEE") {
